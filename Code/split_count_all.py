...
path = r"D:\OneDrive - mail.nankai.edu.cn\桌面\大创\标注体系\标注2.0\1+4+社交+性别" #文件夹目录
files= os.listdir(path) #得到文件夹下的所有文件名称
num_sys=re.compile('<\d+>')
with open("./result_all_data.csv",'a',newline='',encoding='utf-8') as w:
        ...
        with open(position,'r',encoding='utf8') as f:
            ...
            while cont:
                ...
                if cont =='\n':# 读取到空行时，则对此空行到上一空行之间的文本进行提取分析
                    match_tag = re.findall(r"\<\d+\>", str(li)) # 搜索包含数字的对话块
                    if(len(match_tag)>= 1):
                        # w.write(str(li)+'\n')
                        match_result={}
